...
<body class="${pageClass}">
  <header class="site-header">
    ...
    <nav id="site-nav" class="site-nav">
      ${["Home","Word Unscrambler","Anagram Solver","Word Finder","Wordle Solver","Crossword Solver","Dictionary Checker","Random Word Generator","Blog"].map((label,i)=>{
        const href = ["/","/","/anagram-solver","/word-finder","/wordle-solver","/crossword-solver","/dictionary-checker","/random-word-generator","/blog"][i];
...
function wordleTool() {
  return `<section class="tool-card" data-tool="wordle"><form class="wordle-form"><div class="wordle-grid" aria-label="Wordle letter grid">${Array.from({length:5},(_,i)=>`<label>Letter ${i+1}<input name="cell${i}" maxlength="1" autocomplete="off"><select name="state${i}"><option value="unknown">Unknown</option><option value="green">Green</option><option value="yellow">Yellow</option><option value="gray">Gray</option></select></label>`).join("")}</div><label>Correct letters with positions<input name="pattern" placeholder="_ R A _ E" maxlength="9"></label><label>Present letters<input name="present" placeholder="t s"></label><label>Excluded letters<input name="excluded" placeholder="b c d"></label><label class="check"><input type="checkbox" name="hardMode"> Hard mode filtering</label><button type="submit">Find Candidates</button><button type="button" class="secondary" data-action="clear">Clear</button></form><section class="results" aria-live="polite"><h2>Candidates</h2><p class="empty-state">Add green, yellow, gray, or pattern clues to filter five-letter words.</p></section></section>`;
}

function crosswordTool() {
  return `<section class="tool-card" data-tool="crossword"><form class="crossword-form"><label>Crossword pattern<input name="pattern" placeholder="c?a?e or __a_e" autocomplete="off"></label><label>Must include<input name="mustInclude" placeholder="r s t"></label><label>Excluded letters<input name="excluded" placeholder="x z q"></label><button type="submit">Find Matching Words</button><button type="button" class="secondary" data-action="clear">Clear</button></form><section class="results" aria-live="polite"><h2>Matches</h2><p class="empty-state">Enter a pattern with ? or _ for unknown letters.</p></section></section>`;
}

function dailyTool(kind) {
  return `<section class="tool-card" data-tool="daily" data-daily-kind="${kind}"><section class="results" aria-live="polite"><h2>Daily pick</h2><p class="empty-state">Loading today's word activity...</p></section><button type="button" data-action="new-daily">Try another</button></section>`;
}
...
function wordGameCheatSheet() {
  return layout({
    title: "Word Game Cheat Sheet: Patterns, Scores and Letter Tips | LetterForge",
    desc: "A practical word game cheat sheet for finding patterns, using blanks, spotting high-scoring letters, and improving puzzle play.",
    url: "/word-game-cheat-sheet",
    h1: "Word Game Cheat Sheet",
    body: `<section class="narrow-page"><h1>Word Game Cheat Sheet</h1><p>This quick reference is built for word puzzle players who want better habits without memorizing a giant dictionary. Use it before a round, while reviewing a hard rack, or after a puzzle to understand why a word was hard to see.</p><section class="content-section"><h2>Start with word shapes</h2><p>Separate vowels and consonants, then test common shapes such as CVC, CVCC, CCVC, and CVCV. English words often reveal themselves when you stop looking at a rack as random letters and start looking for reusable shapes.</p></section><section class="content-section"><h2>Look for common clusters</h2><p>Useful clusters include sh, ch, th, st, tr, cr, br, pl, sp, and qu. Treat these as building blocks. If a rack contains s, t, a, r, e, try star, stare, tears, rates, and other rearrangements around that cluster.</p></section><section class="content-section"><h2>Watch high-scoring letters</h2><p>Q and Z are worth 10 points, J and X are worth 8, and K is worth 5 in Scrabble-style scoring. Strong plays often come from placing one valuable letter cleanly, not from forcing the longest possible word.</p></section><section class="content-section"><h2>Use blank tiles carefully</h2><p>A blank can complete a strong word, but it can also create too many choices. Add starts with, ends with, length, or must include filters when a wildcard search returns a huge list.</p></section><section class="content-section"><h2>Helpful LetterForge pages</h2><p><a href="/two-letter-words">Two-letter words</a> · <a href="/4-letter-words">4-letter words</a> · <a href="/7-letter-words">7-letter words</a> · <a href="/words-with-q">Words with Q</a> · <a href="/words-containing-ar">Words containing AR</a></p></section>${faqHtml()}<div class="ad-slot" data-ad-slot="content">Ad placeholder</div></section>`
  });
}
...
const seoListPages = [
  ...
  { url:"/words-with-only-vowels", title:"Words With Only Vowels | LetterForge", desc:"Browse vowel-only words and short vowel-heavy word ideas.", h1:"Words With Only Vowels", filter:w=>/^[aeiou]+$/.test(w), intro:"Vowel-only words are uncommon, but useful for learning short plays and unusual vowel patterns." },
  { url:"/palindrome-words", title:"Palindrome Words List | LetterForge", desc:"Browse palindrome words that read the same forward and backward.", h1:"Palindrome Words", filter:w=>w.length > 2 && w === [...w].reverse().join(""), intro:"Palindrome words read the same forward and backward. They are useful for pattern play, word trivia, and vocabulary practice." }
];
...
async function main() {
  ...
  await writePage("/crossword-solver", smallToolPage({ url:"/crossword-solver", title:"Crossword Solver - Find Words by Pattern | LetterForge", desc:"Find crossword answers by pattern using ? or _ for unknown letters, plus included and excluded letters.", h1:"Crossword Solver", intro:"Find words that match crossword-style patterns like c?a?e or __a_e.", tool:crosswordTool(), tips:["Type known letters in their exact positions and use ? or _ for blanks.","This is useful for crossword clues, word ladders, spelling patterns, and any puzzle with fixed letter positions.","Add must-include and excluded letters when a clue gives extra information beyond the pattern."] }));
  ...
  await writePage("/daily-word", smallToolPage({ url:"/daily-word", title:"Random Word of the Day | LetterForge", desc:"Discover a daily word with score, length, letters, and related practice links.", h1:"Random Word of the Day", intro:"A daily word pick for vocabulary practice and quick puzzle warmups.", tool:dailyTool("word"), tips:["Open this page each day for a deterministic daily word from the local list.","It is useful for quick vocabulary prompts, classroom warmups, and returning visitors.","Use the related links to jump into anagrams, word finder, or word details."] }));
  await writePage("/daily-anagram", smallToolPage({ url:"/daily-anagram", title:"Daily Anagram Puzzle | LetterForge", desc:"Try a daily anagram puzzle and reveal possible answers from the LetterForge word list.", h1:"Daily Anagram Puzzle", intro:"Solve a fresh daily anagram puzzle and compare your answer with possible words.", tool:dailyTool("anagram"), tips:["Try solving the scrambled letters before opening the answer list.","This gives visitors a reason to return and creates a light daily puzzle habit.","Use exact anagram mode when you want every answer to use all letters."] }));
  await writePage("/daily-5-letter-word-challenge", smallToolPage({ url:"/daily-5-letter-word-challenge", title:"Daily 5-Letter Word Challenge | LetterForge", desc:"Practice a daily five-letter word challenge with hints and solver links.", h1:"Daily 5-Letter Word Challenge", intro:"Practice one five-letter word each day with hints, score, and related solver links.", tool:dailyTool("five"), tips:["Use the hints first, then reveal the answer when you are ready.","This is useful for Wordle-style practice without copying any official puzzle.","Open the Wordle Solver afterward to explore similar five-letter candidates."] }));
  ...
  for (const page of seoListPages) await writePage(page.url, seoListPage(page));
  await writePage("/word-game-cheat-sheet", wordGameCheatSheet());
  ...
  await writePage("/sitemap", basicPage({ url:"/sitemap", title:"HTML Sitemap | LetterForge", desc:"Browse all main LetterForge pages.", h1:"Sitemap", content:`<ul>${["/","/anagram-solver","/word-finder","/wordle-solver","/crossword-solver","/dictionary-checker","/random-word-generator","/daily-word","/daily-anagram","/daily-5-letter-word-challenge","/word-game-cheat-sheet",...seoListPages.map((page)=>page.url),"/blog","/about","/contact","/privacy","/terms"].map(u=>`<li><a href="${u}">${u}</a></li>`).join("")}</ul>` }));

  const urls = ["/","/anagram-solver","/word-finder","/wordle-solver","/crossword-solver","/dictionary-checker","/random-word-generator","/daily-word","/daily-anagram","/daily-5-letter-word-challenge","/word",...staticWordList.map((word)=>`/word/${word}`),...seoListPages.map((page)=>page.url),"/word-game-cheat-sheet","/blog",...articles.map(a=>`/blog/${a.slug}`),"/about","/contact","/privacy","/terms","/sitemap"];
